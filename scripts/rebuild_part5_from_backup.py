...
def main():
    ...
    print("="*70)
    print("2-4接口文档最终重构 - 重建Part5")
    print("="*70)
    
    # 从备份读取Part2（包含完整的内容）
    backup_part2 = read_file(backup / '2-4_接口文档-Part2-backup.md')
    print(f"\n备份Part2: {len(backup_part2)} 行")
    
    # 找到关键分割点
    data_transform_start = find_line(backup_part2, '## 4. 数据转换与序列化接口')
    plugin_start = find_line(backup_part2, '## 5. 插件扩展接口')
    error_code_start = find_line(backup_part2, '## 6. 错误码详细索引')
    version_mgmt_start = find_line(backup_part2, '## 7. 接口版本管理与兼容性')
    appendix_start = find_line(backup_part2, '## 8. 附录')
    reference_start = find_line(backup_part2, '## 9. 参考文献')
    ...
    print(f"  参考文献: {reference_start}")
    
    # Part5内容：从数据转换到结尾
    part5_content = backup_part2[data_transform_start:]
    ...
    part5_header = [
        ...
        "## 1. 引言\n",
        "\n",
        "本文档为 **AutoFlowCFD** 接口设计规范文档的第5部分（最终部分），涵盖数据转换接口、插件扩展机制、错误码索引和版本管理。\n",
        "\n",
        "**相关文档**:\n",
