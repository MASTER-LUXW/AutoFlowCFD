...
class HighOrderMesh:
    ...
    def __init__(self, order: int = 2):
        ...
        # 四面体体积基函数（Part6/7/8 文档，路径C/native 分支）：
        # 2026-09-03 起恒为 "native"，不再接受 `tet_basis_mode` 参数
        ...
        # 供 fr/face_flux_points/geometry.py 按需重新映射任意计算立方体坐标点到
    ...
    def load_from_volume_mesh(self, volume_mesh_data, build_faces: bool = True):
        ...
        if build_faces:
            ...
            # 只产出坍缩坐标编码（0~5，见该函数及 face_connectivity.py
            # 模块文档"`build_face_connectivity` 因此保持完全不变、只
    ...
    def verify_gcl(self, tolerance: float = 1e-8) -> bool:
        """几何守恒律自检——薄委托到 `gcl_check.verify_gcl`
        （2026-09-19 拆出，见该模块文档）。"""
        from .gcl_check import verify_gcl as _impl

        return _impl(self, tolerance)
    ...
    def get_all_cell_volumes(self) -> np.ndarray:
        ...
        四面体（native 单纯形基）部分**不能**沿用这套张量积求积权重
        （Part8 文档"三、本次会话实现范围"新发现的一个真实坑）：
        `weights_3d` 的和是 8（[-1,1]^3 立方体体积），坍缩坐标下
        ...
        `native_tet/basis.py::compute_native_tet_jacobian` 文档）。
