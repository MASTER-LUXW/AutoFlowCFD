...
def smooth_bad_cells(
    ...
    标准"松弛 + 行搜索阻尼"形式：一遍仅在其引入零个负体积单元时才被
    提交，这是核心安全保证。

    **措辞更正 + 实测依据（2026-09-14）**：此前这里把本方案称为严格
    逐节点接受/拒绝方案的"简化"。那个标注**已被受控实验证伪**——两者
    的最终质量没有可测量差别：

      在定向一致的 Kuhn 立方格四面体网格上（内部节点随机抖动制造坏
      单元），对 9 个算例（n=5/6/7，jitter 0.55~0.95，初始 0~25 个
      负体积单元）分别跑本方案与严格逐节点接受/拒绝方案（后者逐节点
      移动、逐节点只验证受影响邻域、不安全则单独撤回该节点）：
        * **最大边长比 ratio_max：9 个算例全部完全相同**
        * 负体积单元：两者都全部修复为 0（例如 jitter=0.95 的
          18 个、n=7 jitter=0.9 的 25 个）
        * p99 边长比：4/5 个较难算例完全相同，一例严格方案好 1.0%
          （2.577 vs 2.603）
        * 平均边长比：差异只出现在小数第 4 位，两个方向都有（噪声量级）

    原因：整体阻尼只在"某一遍按 relax=1.0 会造出新的负体积单元"时才
    介入；一旦介入，减半后的位移方向与逐节点方案一致，只是幅度统一。
    在这些算例上两者收敛到同样的质量水平。

    因此本方案不是"用质量换便宜"，而是与严格方案**结果等价**、且天然
    可向量化的写法（严格方案必须逐节点循环 + 逐节点重新验证邻域，在
    79 万单元量级的生产网格上开销要高一个量级以上）。

    实验范围如实标注：以上测量在合成格点网格上进行，不是 cube_demo
    生产网格（重建那份体网格代价过高）。cube_demo 质量门仍未通过，但
    其根因已独立追踪到 BL 挤出/棱柱坍缩（见项目记忆
    cube_demo_bl_quality_campaign / mesh_sliver_336_57_root_cause_traced），
    不是本平滑方案的选择问题。
