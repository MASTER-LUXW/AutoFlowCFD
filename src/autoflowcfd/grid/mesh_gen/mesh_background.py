...
# Stage A/B/B' 之后、最终装配之前的混合网格收尾修补阶段，拆到了
# mesh_background_mixed_repair.py（本文件超过 400 行上限，见该模块自己
# 的文档字符串）。
...
def generate_hybrid_mesh(
    ...
    try:
        ...
        # Stage A/B/B' 之后、最终装配之前的混合网格收尾修补阶段（跨类型
        # 非流形面拼接、BL 棱柱畸变长细比局部重铺、collapsed-corner 棱柱
        # 降级为四面体）拆到了 mesh_background_mixed_repair.py（本文件超
        # 过 400 行上限）——三个子步骤共享同一组滚动状态，作为一个整体一
        # 起搬移，逐字对应原来这里的代码，未改动任何数值逻辑。
