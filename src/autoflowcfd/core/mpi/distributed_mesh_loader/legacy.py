"""AutoFlowCFD V2.0 - CPU MPI"传统模式"的网格分发。

从 `core/mpi/distributed_mesh_loader.py` 拆出（2026-09-24）。纯搬家，
逻辑未改。

与 `fully_distributed.py` 的区别：这条路径上**每个 rank 都加载完整网格**
再各自取本地部分；`fully_distributed` 只有 root 加载完整网格。两条都在用，
不是新旧替代关系。
"""
...
from autoflowcfd.core.mpi import get_comm, get_mpi, get_rank
