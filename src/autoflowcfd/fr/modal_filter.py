...
曾经存在的残差量级离群抑制（`suppress_residual_outliers`，"机制3"，
已于 2026-09-19 整体删除）无法压制这种失稳——噪声放大是同一个单元里
*大面积、渐变分布*的（不是单个 SP 异常凸出），跟同单元其余 SP 的中位数
比没有显著的相对倍数差异，那条"单元内相对异常"判据结构性地检测不到。
这条推理在当年就是本模块存在的部分理由；机制3 的删除依据（真实网格
消融对照）见 `core/fr_residual/inviscid.py`，与本段结论方向一致。
...
#   P1     退化成 P0（阶 1）                    达到设计阶 2
#   P2     legacy 非幂等、约 100 步退到 P0；     达到设计阶 3
#          project 精确降一阶（阶 2 vs 设计 3）
#
# **P2 那一行已于 2026-09-20 更正**：原文写的是"只在被标记的约 17.6%
# 单元里降阶，全局阶约 2.1"，并据此得出"门控默认只适合 P1"。那组数字是
# 用**面邻居**包络测的，而生产单机路径自 2026-09-19（C2 修复）起用的是
# **顶点邻居**包络。换成顶点模板重测（等熵涡精确解，nx=8->48）：
#
#   棱柱基  order  标记比例            project 档收敛阶     设计阶
#   坍缩    P2     18.75% -> 0.260%    2.00 / 2.97 / 3.63    3
#   原生    P2     18.75% -> 0.608%    1.94 / 2.73 / 3.12    3
#
# 也就是说"BJ 判据与阶数有偏"是**面模板的性质**，不是判据本身的性质；
# 门控在 P2 上同样渐近失活、同样达到设计阶。完整表与两条基的差别见
# `tests/validation/test_bounds_gate_accuracy.py` 的模块文档。
#
