...
**源项本身是正常的截断误差**（2026-09-18 更正）。把同一个**处处光滑**
的 z 无关解析场投影到各网格上直接量残差的 w 分量（无时间积分、无暂态）：

    P1: L2 阶 +1.71 / +1.83    max 阶 +1.61 / +1.75   （设计阶 1）
    P2: L2 阶 +2.69 / +2.82    max 阶 +2.62 / +2.76   （设计阶 2）

收敛，而且好于设计阶。**所以问题不在源项，而在离散系统对它的响应**：
展向动量在这个准二维设置里几乎没有恢复机制（两侧对称面不可穿透），
一个持续的小源项就会累积 —— P1 上饱和、P2 上无界增长。

（两次被否掉的错误测量，记在这里当方法论教训：① 在固定伪时间下比不同
网格的 `max|w|`，而那时各网格的解处于**不同暂态**；② 改用解析场但选了
Blasius 剖面，被前缘 `sqrt(x)` 奇点污染。截断误差的阶必须用光滑场量。）
...
_STEPS = 750
_NX = 16
#: CFL **改用项目默认值 0.03**（2026-09-19，原值 0.1）。
#:
#: 原值 0.1 高于项目自己的默认上限 `cfl_max=0.06`（2026-09-17 按实测
#: 重定），它当初能跑下来靠的是模态滤波器的**人工耗散** —— 默认档
#: 2026-09-19 改成 `off` 之后，挤出轴那个对照配置在 CFL 0.1 下第 652 步
#: 出 NaN，而在 0.03 与 0.06 下全程有限。实测（挤出轴对照，750 步）：
#:
#:     CFL    off                     sensor
#:     0.03   全程有限，展向 2.1e-12%   全程有限，展向 3.9e-12%
#:     0.06   全程有限                 全程有限
#:     0.10   **第 652 步 NaN**        全程有限
#:
#: 也就是说：拿一个高于默认上限的 CFL 去测，实际上是在测"滤波器的人工
#: 耗散够不够撑住这个步长"，而不是在测本文件要判定的那条离散对称性。
#: 用默认 CFL 之后两档都稳，判据回到它该判的东西上。
_CFL = 0.03
...
#: 默认档下 P2 的 |w| 上限（相对来流）。实测 400 步 5.25e-02。
_P2_W_MAX = 0.10
...
class TestP2OnACleanPrismMesh:
    """P2 在干净结构化棱柱网格上的稳定性 —— **缺陷已于 2026-09-20 修复**。

    本类原名 `TestP2DivergesOnACleanPrismMesh`，钉的是"P2 必须在 150 步内
    发散"这个已知缺陷；它的文档当时写明"任何针对 P2 发散的修复都应当先在
    这上面验证，而这条测试会在修复生效的那一刻失败，强制更新文档与判据"。
    **那一刻到了**：原生棱柱基（`AFCFD_PRISM_BASIS=native`，2026-09-20 起
    是默认）让同一算例在同一 CFL 下跑完 400 步不发散。

    实测（同一张网格、同一 CFL 0.1、同一默认档）：

        坍缩基 P2   第 82 步非有限（发散）
        原生基 P2   400 步全程有限，max|w|/U = 5.25e-02（150 步时 4.11e-02）
        原生基 P1   见下面那条，|w| 同样有界

    所以本类现在是**正向判据**（默认档下 P2 必须稳定）+ 一条显式跑在
    坍缩档上的**负控制**（把"修好了什么"写成可执行的对照，而不是删掉
    历史记录）。伪横流本身**没有**消失：原生档 400 步 |w|/U 仍有 5.25%，
    与 P1 的饱和值同量级 —— 修好的是"无界增长"，不是"伪横流归零"，
    这条区别必须如实写在判据里。

    ## 为什么这个算例值得留着（原文保留）

    plate_demo 363k 的 P2 重测每步 150 s、第 68 步发散，调试循环极慢；
    同一个失效在这里 1152 单元、**一分钟一轮**就能复现。这张网格
    **全是规整棱柱、无退化单元、均匀分布**，算例是**层流**且有精确解
    —— 所以它同时排除了两条常被怀疑的成因：网格质量、湍流模型。正因为
    如此，它才能把"病根在棱柱基本身"这个结论逼出来。

    ## 缺陷期的实测记录（坍缩档，2026-09-18）
    ...
    P2 上**无界增长**，差别只在阶数 —— 这两条合起来正是"换基"而不是
    "调参"才能解决的特征。（2026-09-20 复测坍缩档是第 82 步，与当时的
    75 步差别来自滤波默认档在那之后改成了 off。）
    """

    def test_p2_is_stable_on_the_default_basis(self):
        """**正向判据**：默认档（原生棱柱基）下 P2 必须跑完全程且 |w| 有界。

        实测 400 步 max|w|/U = 5.25e-02。判据用 `_P2_W_MAX = 0.10`（与 P1
        同一档上限），既不把实测值当阈值，也不松到没有意义。
        """
        step, worst = _run_until_nonfinite(2, _NX, 0.1, _P2_MUST_SURVIVE)
        assert step is None, (
            f"默认档下 P2 第 {step} 步发散 —— 原生棱柱基修好的正是这一条"
            f"（见本类文档的对照数据），发散说明有回归。")
